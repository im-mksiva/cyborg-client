...
class DeviceManager(Module.Module):
    ...
    def __init__(self, board, session):
        ...
        # testing with the cyborg client
        transport = messaging.get_rpc_transport(cfg.CONF)
        target = messaging.Target(topic='cyborg-agent', version='1.0')
    ...
    async def CustomMethod(self, req, parameters):
        ctx = context.RequestContext()
        req_id = req['uuid']
        rpc_name = utils.getFuncName()
        LOG.info(req)
        LOG.info("RPC " + rpc_name + " CALLED [req_id: " + str(req_id) + "]:")
        result = self.client.call(ctx,'driver_creation')
        LOG.info(result)
        w_msg = WM.WampSuccess(msg="helloooooo",req_id=req_id)
